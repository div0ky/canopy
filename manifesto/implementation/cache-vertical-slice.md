...
Canopy owns an injectable `Cache` port with `get`, `put`, atomic `add`, atomic `increment`,
`forget`, and `remember` semantics. Values use Canopy's JSON contract, and TTL is expressed in
seconds.
...
in `canopy_cache_entries`, replaces expired entries atomically, preserves TTL across increments, and
