...
- `Dockerfile`: pinned Node 24, pnpm, multi-stage build, production dependencies, non-root runtime,
  precompiled Canopy artifacts, and a web default command.
...
- `SIGTERM` reaches Arbor so Canopy drains before shutdown.
- Undergrowth remains disabled in production unless explicitly overridden.
...
7. Cultivate reports the same topology and invariants as generated files.
