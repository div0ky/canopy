# 0003: Build Authentication as a First-Party Canopy Subsystem
...
behavior, schema, lifecycle, or upgrade changes on a boundary Canopy promises to keep coherent.

Wrapping a full authentication framework would also reproduce the architectural problem that led
Canopy away from NestJS: the dependency would own important application semantics beneath a
Canopy-branded facade.

Canopy therefore accepts responsibility for the complete authentication contract and for safely
...
Delegated code must not define Canopy's database schema, public types, routes, session model, actor
model, or lifecycle.
...
11. Canopy-owned test helpers and diagnostics.

OAuth/OIDC, passkeys, multifactor authentication, magic links, API keys, and machine identities are
not part of the core authentication implementation. They may be installed as optional Canopy auth
...
Every plugin must integrate through Canopy-owned extension points for:
...
- HTTP or console entry points expressed through Canopy manifests.
- Configuration, secret handling, and diagnostics.
- Security events and audit metadata.
- Test fakes and conformance scenarios.

A plugin may return a verified Canopy identity. It must never construct the application `Actor`
...
Initially, Canopy will ship and support first-party auth plugins only. A third-party plugin API can
be opened later if provenance, compatibility, permissions, migrations, and security review can be
made trustworthy.

Expected optional plugins include:

```text
canopy add auth:oauth
canopy add auth:passkeys
canopy add auth:mfa
canopy add auth:api-keys
...
Bearer access tokens represent an existing Canopy identity. API keys and machine credentials that
...
dependency is unavoidable, Canopy will:

- Keep it behind a narrow Canopy-owned contract.
- Pin the exact version in a Canopy release.
...
feature set. It is rejected as Canopy's foundational authentication engine because its schema,
sessions, endpoints, plugins, and upgrade path would define too much of Canopy's security-critical
application behavior.

Better Auth remains useful comparative evidence when designing flows and conformance cases, but
Canopy will not wrap it as the default implementation.

## Consequences

- Canopy controls authentication behavior, schema evolution, and release compatibility.
...
- Hosted enterprise identity providers may later integrate through optional federation plugins
  without replacing Canopy identities, sessions, actors, or policies.
...
10. Canopy-owned fakes that do not weaken production semantics accidentally.
...
- Canopy cannot sustain the security maintenance and response obligations this decision creates.
- A standards or platform change makes the selected credential or session model unsafe.
- A delegated primitive begins determining Canopy's public behavior or stored semantics.
- A hosted identity requirement cannot be integrated through a plugin without replacing the Canopy
...
- [Canopy Manifesto: framework boundary](../index.md#the-framework-boundary)
