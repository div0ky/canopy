# 0019: Defer Doxa Decorator Syntax
...
The Doxa MVP does not support decorators as a framework declaration syntax. The primary and only MVP
authoring model uses class roles, Feature role arrays, capability interfaces, constructor types,
explicit stable IDs, declarative bindings, and typed handler methods.

Optional Doxa decorators are deferred rather than permanently rejected.

## MVP boundary

The Doxa compiler does not interpret decorators for framework semantics. It does not depend on
...
TypeScript toolchain, but those decorators do not change the Doxa manifest.
...
- CLI, diagnostics, tests, and Gnosis report identical semantics regardless of syntax.
