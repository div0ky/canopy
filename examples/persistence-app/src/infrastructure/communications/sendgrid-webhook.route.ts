...
export class SendGridWebhookRoute extends Route {
  ...
  async handle(request: HttpRequest): Promise<Response> {
    ...
    if (!publicKey)
      throw new HttpError(503, 'webhook_not_configured', 'The SendGrid webhook is not configured.')
    if (!verifySendGridWebhook(raw, timestamp, signature, publicKey.reveal())) {
      throw new HttpError(
        403,
        'invalid_webhook_signature',
        'The SendGrid webhook signature is invalid.',
      )
    }
    if (Math.abs(Date.now() / 1_000 - Number(timestamp)) > 300) {
      throw new HttpError(
        403,
        'stale_webhook',
        'The SendGrid webhook timestamp is outside the accepted window.',
      )
    }
