...
export class RevokeSessionRoute extends Route {
  static override readonly id = 'revoke-session'; static override readonly access = 'accounts.sessions.manage'
  readonly method = 'DELETE'; readonly path = '/auth/sessions/:id'
  ...
  async handle(request: HttpRequest): Promise<Response> {
    ...
    return new Response(null, { status: 204, ...(id === this.execution.context.authentication.sessionId ? { headers: { 'set-cookie': this.auth.expiredSessionCookie() } } : {}) })
  }
}
