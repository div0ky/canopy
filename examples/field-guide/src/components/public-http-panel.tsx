...
export function PublicHttpPanel({ connected, onHello }: PublicHttpPanelProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    setPending(true)
    try {
      setResponse(await onHello(name))
    } catch {
      setResponse('The request failed. See the notification for details.')
    } finally {
      setPending(false)
    }
  }
        ...
        <form onSubmit={submit}>
          <FieldGroup>
            ...
            <Field>
              ...
              <output
                id="hello-response"
                className="min-h-20 rounded-lg border bg-muted/45 p-4 font-mono text-sm text-primary"
              >
                {response}
              </output>
            </Field>
