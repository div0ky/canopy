  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Protected action that mutates a model and queues durable work.</CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-5">
        <Field>
          <FieldLabel htmlFor="counter-id">Specimen ID</FieldLabel>
          <Input id="counter-id" value={id} onChange={(event) => setId(event.target.value)} className="font-mono" required />
        </Field>
        ...
        <Field>
          ...
          <div className="grid grid-cols-[auto_1fr_auto] overflow-hidden rounded-lg border bg-background">
            <Button type="button" variant="ghost" size="icon-lg" className="rounded-none border-r" onClick={() => setAmount((current) => Math.max(1, current - 1))} aria-label="Decrease amount"><MinusIcon aria-hidden="true" /></Button>
            <output className="flex items-center justify-center font-mono">{amount}</output>
            <Button type="button" variant="ghost" size="icon-lg" className="rounded-none border-l" onClick={() => setAmount((current) => current + 1)} aria-label="Increase amount"><PlusIcon aria-hidden="true" /></Button>
          </div>
        </Field>
        <Button
          type="button"
          ...
          onClick={async () => {
            setPending(true);
            try { await onIncrement(id, amount); } catch { /* The shell reports normalized Canopy errors. */ } finally { setPending(false); }
          }}
        >
