...
import type { CompileApplicationResult } from '@canopy/compiler'
import { type ActionClass, ObservationRecorder } from '@canopy/core'
import { HonoHttpEngine } from '@canopy/http-hono'
import { Canopy, type CanopyRuntime } from '@canopy/runtime'
import { listenUndergrowth, pruneUndergrowth, UndergrowthStore } from '@canopy/undergrowth'
...
export function registerCompilationAndUndergrowthTests(
  ...
  it('compiles Unit of Work and transaction capabilities without engine types', async () => {
    ...
    expect(result.manifest.providers).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: 'provider:infrastructure/undergrowth',
  ...
  it('records correlated typed observations and exposes a read-only execution timeline', async () => {
    ...
    await waitFor(
      async () =>
        Number(
          (await pool().query('SELECT count(*) AS count FROM canopy_undergrowth_observations'))
            .rows[0]?.count,
        ) >= 4,
    )
  ...
  it('keeps application execution independent from observation recorder failure', async () => {
    ...
    const runtime = await Canopy.boot(Application, {
      ...
      providerOverrides: {
        'provider:infrastructure/undergrowth': new FailingObservationRecorder(),
      },
    })
    runtimes.push(runtime)
    const response = await new HonoHttpEngine(runtime).fetch(
      new Request('http://canopy.test/health'),
    )
  ...
  it('prunes Undergrowth deterministically by count and age', async () => {
    ...
    expect(
      await pruneUndergrowth(connectionString(), { retentionDays: 7, maximumObservations: 2 }),
    ).toBe(2)
    expect(
      Number(
        (await pool().query('SELECT count(*) AS count FROM canopy_undergrowth_observations'))
    ...
    await pool().query(
      `UPDATE canopy_undergrowth_observations SET occurred_at = now() - interval '8 days' WHERE name = 'entry-2'`,
    )
    expect(
      await pruneUndergrowth(connectionString(), { retentionDays: 7, maximumObservations: 10 }),
    ).toBe(1)
  })

  it('serves the read-only Undergrowth explorer from its dedicated loopback host', async () => {
    const host = await listenUndergrowth({ connectionString: connectionString(), port: 0 })
    try {
    ...
    await expect(
      listenUndergrowth({ connectionString: connectionString(), host: '0.0.0.0', port: 0 }),
