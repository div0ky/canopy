...
export class PostgresUndergrowth extends ObservationRecorder implements Starts, Drains, Disposes {
  ...
  constructor(private readonly options: PostgresUndergrowthOptions) { super() }
  ...
  async start(context: LifecycleContext): Promise<void> {
    ...
    if (environment === 'production' && this.options.allowProduction !== true) {
      throw new Error('Undergrowth is disabled in production unless allowProduction is explicitly true.')
    }
    const pool = new Pool({
  ...
  record(observation: Observation): void {
    ...
    if (!pool) return
    this.#writes += 1
    const write = this.#tail.then(() => insertObservation(pool, observation))
      .then(async () => {
        if (this.#writes % 500 === 0) {
          await pruneWithPool(pool, this.options.retentionDays ?? 7, this.options.maximumObservations ?? 50_000)
        }
      })
      .catch(() => undefined)
    this.#tail = write
...
export async function pruneUndergrowth(
  ...
  try { return await pruneWithPool(pool, options.retentionDays ?? 7, options.maximumObservations ?? 50_000) }
...
async function insertObservation(pool: Pool, observation: Observation): Promise<void> {
  ...
  `, [
    observation.id, observation.occurredAt, observation.kind, observation.name, observation.phase,
    observation.roleId ?? null, observation.durationMilliseconds ?? null,
    context.executionId ?? null, context.sourceExecutionId ?? null, context.correlationId ?? null,
    context.causationId ?? null, context.traceId ?? null, context.spanId ?? null,
    context.actorKind ?? null, context.actorId ?? null, context.tenantId ?? null,
    context.transport ?? null, context.transportName ?? null,
    JSON.stringify(observation.attributes), observation.error ? JSON.stringify(observation.error) : null,
  ])
}
...
function validateRetention(options: PostgresUndergrowthOptions): void {
  ...
  if (options.maximumObservations !== undefined
