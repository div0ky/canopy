...
<main class="workspace"><section class="pane"><div class="pane-head"><h2 id="rail-title">Executions</h2><span id="count">0</span></div><div class="filters" id="filters"><button class="chip active" data-kind="" data-phase="">All</button><button class="chip" data-kind="http">HTTP</button><button class="chip" data-kind="job">Queue</button><button class="chip" data-kind="event">Events</button><button class="chip" data-kind="schedule">Schedules</button><button class="chip" data-phase="failed">Failed</button></div><div class="error-banner" id="error"></div><div class="scroll" id="executions"><div class="empty"><strong>Waiting beneath the surface</strong>Run your Canopy application and executions will appear here.</div></div></section>
...
async function loadExecutions(){try{el('error').style.display='none';const p=new URLSearchParams();if(state.kind)p.set('kind',state.kind);if(state.phase)p.set('phase',state.phase);if(state.search)p.set('search',state.search);state.executions=await api((state.kind?'/api/entries?':'/api/executions?')+p);const selected=state.executions.find(x=>(x.entryId||x.executionId)===(state.entry||state.execution));if(!selected){state.execution=null;state.entry=null}renderExecutions();if(!selected&&state.executions[0])await chooseExecution(state.executions[0].executionId,state.executions[0].entryId)}catch(e){el('error').textContent=e.message;el('error').style.display='block'}}
function renderExecutions(){const titles={http:'HTTP',job:'Queue',event:'Events',schedule:'Schedules'};el('rail-title').textContent=titles[state.kind]||'Executions';el('count').textContent=state.executions.length;el('executions').innerHTML=state.executions.length?state.executions.map(x=>'<button class="execution '+((x.entryId||x.executionId)===(state.entry||state.execution)?'active ':'')+(x.phase==='failed'?'failed':'')+'" data-id="'+esc(x.executionId)+'" data-entry="'+esc(x.entryId||'')+'"><div class="execution-top"><span class="status '+(x.phase==='failed'?'failed':'')+'"></span><span class="kind">'+esc(x.kind||x.transport||'run')+'</span><span class="execution-name">'+esc(x.name)+'</span></div><div class="execution-meta"><span>'+new Date(x.occurredAt).toLocaleTimeString()+'</span><span>· '+esc(x.phase)+'</span><span class="duration">'+fmt(x.durationMilliseconds)+'</span></div></button>').join(''):'<div class="empty"><strong>No '+esc((titles[state.kind]||'executions').toLowerCase())+' recorded</strong>Run your Canopy application and matching evidence will appear here.</div>';document.querySelectorAll('.execution').forEach(b=>b.onclick=()=>chooseExecution(b.dataset.id,b.dataset.entry||undefined))}
