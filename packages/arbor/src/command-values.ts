...
export function integerOption(args: readonly string[], name: string, fallback: number): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 0 || parsed > 65_535)
    throw new ArborCommandError(`--${name} must be an integer from 0 through 65535.`)
  return parsed
}
...
export function numberOption(args: readonly string[], name: string, fallback: number): number {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0)
    throw new ArborCommandError(`--${name} must be a positive number.`)
  return parsed
}
...
export function positiveIntegerOption(
  ...
  if (!Number.isInteger(parsed) || parsed <= 0)
    throw new ArborCommandError(`--${name} must be a positive integer.`)
  return parsed
}
