...
import type { QueueEnvelope } from '@canopy/core'
...
Runtime commands reserved by the MVP contract:
  serve  work  schedule  dev  migrate  migrate:status  test
`
...
async function makeApplication(directory: string, rawName: string): Promise<void> {
  ...
  const files: Readonly<Record<string, string>> = {
    ...
    'src/infrastructure/telemetry.ts': `import { Telemetry, type TelemetryRecord } from '@canopy/core'\n\nexport class ApplicationTelemetry extends Telemetry {\n  static id = 'telemetry'\n  record(record: TelemetryRecord): void { console.log(JSON.stringify(record)) }\n}\n`,
    'src/infrastructure/infrastructure.feature.ts': `import { Feature } from '@canopy/core'\nimport { ApplicationAuth } from './auth.js'\nimport { ApplicationCache } from './cache.js'\nimport { DatabaseConfig } from './database.config.js'\nimport { ApplicationMail } from './mail.js'\nimport { Queues } from './queues.js'\nimport { ApplicationSms } from './sms.js'\nimport { ApplicationTelemetry } from './telemetry.js'\nimport { Transactions } from './transactions.js'\n\nexport class InfrastructureFeature extends Feature {\n  id = 'infrastructure'\n  configs = [DatabaseConfig]\n  providers = [Transactions, Queues, ApplicationAuth, ApplicationCache, ApplicationMail, ApplicationSms, ApplicationTelemetry]\n}\n`,
...
async function writeCultivateKnowledge(cwd: string, manifest: Record<string, unknown>): Promise<void> {
  ...
  const knowledge = {
    ...
    arbor: {
      ...
      operations: ['migrate', 'migrate:status', 'queue:list', 'queue:failed', 'queue:retry', 'queue:cancel', 'delivery:list', 'delivery:retry'],
...
async function runRuntimeCommand(
  ...
  if (command === 'dev') {
    await withDatabase(cwd, args, async (pool) => { await applyMigrations(pool, await discoverMigrations(cwd)) })
  }
  const result = await buildApplication(cwd)
  ...
  const worker = command === 'work' || command === 'dev'
  const scheduler = command === 'schedule' || command === 'dev'
...
async function runApplicationCommand(name: string, args: readonly string[], cwd: string): Promise<boolean> {
  ...
  const runtime = await Canopy.boot(module.Application, {
    artifactsDirectory: path.join(cwd, '.canopy'),
    dotenvPath: false,
    environment: { ...await dotenvEnvironment(cwd), ...process.env },
...
function runProcess(command: string, args: readonly string[], cwd: string): Promise<number> {
